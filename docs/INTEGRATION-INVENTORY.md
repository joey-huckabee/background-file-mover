...
**Status: Phase 1, built and verified once (2026-08-08).** A Rocky 9.8 guest
installs unattended, boots, takes a DHCP lease and accepts SSH key auth as
`labadmin`, with SELinux `Enforcing` and passwordless sudo. Layer 2 — the
Ansible playbook and `tests/run-all.sh` — has **not** run. See § 7 for the line
between what is verified and what is merely written.
...
| 1.8 | Signing key pin | `21CB256AE16FC54C6E652949702D426D350D275D` in `provision/hyperv/rocky-gpg-fingerprint.txt` | Rocky Enterprise Software Foundation Release key 2022. Until the fingerprint is confirmed against a source outside the download channel, 1.7 is trust-on-first-use rather than verification; the script refuses to invent a fingerprint, because a wrong pin looks like verification and is not. **Confirmed and pinned 2026-08-08.** | **Manual, once** | re-run 1.7: it reports `fingerprint matches the pin` |
...
| 3.10 | Finding the guest's IP | integration services first, **ARP by MAC** as fallback | `Get-VMNetworkAdapter().IPAddresses` is populated by the guest's KVP daemon, from `hyperv-daemons` — **not** in `@^minimal-environment`, and the lab does not install it. On the first real build a 25-minute watcher polled 50 times and got an empty address every time, while the VM was up, networked and answering SSH for fifteen of those minutes. `Get-TestLab.ps1` would have said "the install is probably still running" indefinitely. It now falls back to matching the VM's Hyper-V MAC in the host neighbour cache, sweeping the lab switch's /24 first if the cache is cold, and reports **which** method found the address. | `Get-TestLab.ps1` | it prints `Address : <ip>  (via …)` |
| 3.11 | Install media after install | detach both DVDs | The boot order puts the DVD first, so media left attached is a reinstall loop waiting for a reboot. `reboot --eject` and the UEFI boot entry Anaconda writes both work against that, but neither is something to rely on. Detaching also frees `filemover-ks.iso`, which Hyper-V holds open while attached — `New-KickstartIso.ps1` cannot replace it otherwise, and now says so by name instead of reporting a bare IOException. | **Manual, after install** | `Get-VMDvdDrive fm-rocky9-01` shows no paths |
...
| 4.2 | Firewall | enabled, `ssh` requested | A lab with the firewall off cannot tell "not listening" from "blocked" — a distinction an operator faces on a real deployment. **Observed divergence:** the running zone is `ssh cockpit dhcpv6-client`. `firewall --enabled --service=ssh` *adds* to firewalld's default `public` zone rather than replacing it, and `cockpit` and `dhcpv6-client` are in that default. Recorded rather than silently accepted; not currently narrowed, since neither reaches the control-plane port the tests care about. | `firewall-cmd --list-services` |
| 4.3 | Root account | `--lock` | A password-less root on a LAN-visible VM is unacceptable even in a lab. | `passwd -S root` |
| 4.4 | Lab account | `labadmin`, wheel, SSH key **plus** a console password | Key auth over the network; a password for the console only. The first build locked the password too, which was stricter and meant that had sshd failed to start there would have been **no way into the machine at all** — the "VM with no console" problem, applied to ourselves. A lab that must be rebuilt to be diagnosed destroys the evidence you wanted to look at. | `ssh -i <key> labadmin@<ip>`; password login at `vmconnect` |
| 4.4.1 | Console password | 24 chars, generated, in `D:\filemover-lab\keys\console-password.txt` | Generated by `New-KickstartIso.ps1` from a CSPRNG, stored beside the SSH private key, owner-only ACL, **never in git**. Alphabet excludes `l1I0O` because it gets typed at a VM console from a screenshot. Reused if it already exists, for the same reason the SSH key is: regenerating locks you out of a VM built from the previous ISO. `--plaintext` in the kickstart because the substitution happens on Windows, which has no `crypt(3)`; the ISO carrying it sits in the directory that already holds the private key. | `New-KickstartIso.ps1` | the file exists; the ISO's `ks.cfg` contains it |
| 4.4.2 | SSH stays key-only | `/etc/ssh/sshd_config.d/99-lab-keyonly.conf` — `PasswordAuthentication no`, `KbdInteractiveAuthentication no`, `PermitRootLogin no` | Without this, giving `labadmin` a console password would silently make it a **network** password too, turning the account from "unreachable without the key" into "guessable from the LAN" with no line saying so. In the kickstart rather than Ansible — against this section's own rule — because it must hold at **first boot**; applying it in the playbook leaves a window whose length depends on when someone gets round to running Ansible. | `%post` in the kickstart | `sshd -T \| grep passwordauthentication` |
...
| **The kickstart ISO is correct** | Built, mounted, and inspected: label `OEMDRV`, one file `ks.cfg`. The **rendered** file — the exact bytes Anaconda reads — passes `ksvalidator -v RHEL9`, contains the generated console password, contains **no** surviving `@KEY@`/`@CONSOLEPW@` placeholder, and is 0 CR bytes with no BOM. |
| **The guest itself, once** | Installed unattended and reached: hostname `fm-rocky9-01`, DHCP lease on the LAN, SSH key auth as `labadmin`, `getenforce` → **`Enforcing`**, `sudo -n true` → 0, firewalld running, `python3` and `openssh-server` present, `multi-user.target`. |
...
| **Layer 2** | The Ansible playbook and `tests/run-all.sh` have never run against the guest. `ansible-lint` and `--syntax-check` pass, which says nothing about whether the roles do what they claim. |
| **The console password path** | Generated, substituted into the ISO and confirmed present in the rendered `ks.cfg` — but the VM currently running was installed **before** it existed, so no one has yet logged in at a console with it. First rebuild proves it. |
| **`Get-TestLab.ps1`'s subnet sweep** | The ARP *match* is verified against the live VM. The **cold-cache sweep** branch has not executed: clearing the host neighbour cache needs elevation, and the entry was already present. It will first run on a rebuild. |
| **Scratch files at fixed `/tmp` paths** | Fixed. Worth recording because of how it presented: `verify-iso-signature.sh` wrote to `/tmp/gpgverify.err`, one `sudo` run left that owned by root, and every later run as a normal user then failed its redirect — which `set -e` turned into `SIGNATURE VERIFICATION FAILED`, illustrated with the *previous* run's file, which said `Good signature`. A false negative whose evidence was proof of success. All three scripts now use `mktemp -d` with a trap. |
