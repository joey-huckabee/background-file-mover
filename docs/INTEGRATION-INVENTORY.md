...
| 1.5 | Install media | Rocky 9 **minimal** ISO in `D:\filemover-lab\iso` | Self-contained base install, so the install itself does not depend on a mirror. Not downloaded automatically: 2.5 GB, mirror URLs rot, and a script that silently fetches an unverified OS image is not something this project should own. | **Manual** | `New-TestLab.ps1` fails with a clear message if absent |
