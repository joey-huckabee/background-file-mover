...
| L2-JOB-015 | _(none)_ | `cpp/tests/test_store.cpp::[a failed sequence bump rolls back and issues nothing]`<br>`cpp/tests/test_store.cpp::[the sequence does not restart across a reopen]`<br>`cpp/tests/test_store.cpp::[the sequence is monotonic within a process]` | Implemented |
...
| L2-SEC-004 | _(none)_ | `cpp/tests/test_fsops.cpp::[an absent entry is an answer, not a failure]`<br>`cpp/tests/test_fsops.cpp::[entries are classified by type before anything acts on them]`<br>`cpp/tests/test_fsops.cpp::[enum names round-trip to stable tokens]`<br>`cpp/tests/test_fsops.cpp::[only regular files are opened]` | Implemented |
